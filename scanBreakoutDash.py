...
def display_flash_cards(matches, potential):
    ...
    st.markdown('<div class="section-header">📱 Flash Cards - Best Opportunities</div>', unsafe_allow_html=True)
...
def display_dashboard():
    ...
    with col5:
        # Count double bottom patterns
        db_count = sum(
            1 for m in matches 
            if m.get('timeframes', {}).get(m.get('best_timeframe', ''), {}).get('pattern', {}).get('has_double_bottom', False)
        )
        db_count += sum(
            1 for p in potential 
            if p.get('pattern', {}).get('has_double_bottom', False)
        )
        st.markdown(summary_card("DOUBLE BTM", db_count, "W patterns found", "summary-card-orange"), unsafe_allow_html=True)
    ...
    # Tabs
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📱 Flash Cards", "🏆 Top Opportunities", "🎯 Confirmed Breakouts", "⏳ Potential Setups", "📊 Full Analysis"])
...
def display_confirmed(matches):
    """Display confirmed breakouts grouped by upside buckets with target status."""
    st.markdown('<div class="section-header">🎯 Confirmed Breakouts by Upside Potential</div>', unsafe_allow_html=True)
    ...
    # Summary metrics
    st.markdown("### 📊 Upside Distribution (Remaining to Targets)")
    ...
    for bucket in buckets:
        ...
        for p in bucket['items']:
            ...
            if p['targets_pending'] > 0 and p['next_target']:
                title = f"**{p['symbol']}** ({p['tf']}) — {status} | Next: ${p['next_target']['price']:.2f} (+{p['next_target_pct']:.1f}%) {indicator_str}"
            else:
            ...
            with st.expander(title, expanded=False):
                ...
                with col1:
                    ...
                    st.write(f"Stop: ${p['stop_loss']:.2f}")
                ...
                with col3:
                    st.markdown("**Status**")
                    st.write(f"Targets Met: {p['targets_met']}/{p['total_targets']}")
                ...
                with col4:
                    ...
                    if p['has_double_bottom']:
                        st.write(f"Dbl Btm: {'✅ Confirmed' if p['double_bottom_confirmed'] else '📊'}")
                    pattern_score = p['pattern'].get('pattern_score', 0)
                    st.write(f"Pattern: {pattern_score}/100")
                
                if st.button(f"📈 View Chart", key=f"conf_{p['symbol']}_{p['tf']}"):
...
def display_potential(potential):
    """Display potential breakouts grouped by upside buckets with MACD filter."""
    st.markdown('<div class="section-header">⏳ Potential Breakouts (MACD Bullish)</div>', unsafe_allow_html=True)
    
    if not potential:
        st.info("No potential setups found with bullish MACD (uptick or positive)")
        return
    
    # Calculate upside for each potential if not already present
    for p in potential:
    ...
    for bucket in buckets:
        ...
        st.markdown("")  # Spacing between buckets
