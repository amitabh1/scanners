...
def opportunity_card_html(symbol, name, sector, industry, score, upside, rr, 
                          ...
                          volume_strong=False, double_bottom=False, is_breakdown=False):
    """Create creative opportunity card HTML with inline styles."""
    
    # Card border color - different for breakdown
    if is_breakdown:
        if upside > 25:
            border_color = '#c31432'  # Dark red for big downside
        elif card_type == 'confirmed':
            border_color = '#f44336'  # Red for confirmed breakdown
        else:
            border_color = '#FF9800'
    else:
        if upside > 25:
    ...
    # MACD pill color - different for breakdown
    if is_breakdown:
        if '↓' in macd_status or 'strong' in macd_status.lower():
            macd_bg = '#f44336'
            macd_text = 'white'
        elif '-' in macd_status or 'negative' in macd_status.lower():
            macd_bg = '#ffebee'
            macd_text = '#c62828'
        else:
            macd_bg = '#e3f2fd'
    ...
    if volume_strong:
        indicators.append('🔥')
    if is_breakdown:
        if double_bottom:  # This would actually be double_top for breakdown but reusing variable
            indicators.append('M')  # M for double top
    else:
        if double_bottom:
            indicators.append('W')
    ...
    if score >= 70:
        gauge_color = '#4CAF50' if not is_breakdown else '#f44336'
    elif score >= 50:
        gauge_color = '#FF9800'
    else:
        gauge_color = '#f44336' if not is_breakdown else '#4CAF50'
    ...
    # Signal gauge - different color and sign for breakdown
    ...
    html += f'</div>'
    if is_breakdown:
        html += f'<span style="font-size:13px;font-weight:600;color:#f44336;">-{upside:.1f}%</span>'
    else:
    ...
    # For breakdown, target is below entry, stop is above
    target_color = '#f44336' if is_breakdown else '#4CAF50'
    stop_color = '#4CAF50' if is_breakdown else '#f44336'
    ...
    html += f'</td>'
    html += f'<td style="background:#f8f9fa;padding:10px;border-radius:8px;text-align:center;width:25%;">'
    html += f'<div style="font-size:15px;font-weight:700;color:{target_color};">${target:.2f}</div>'
    html += f'<div style="font-size:9px;color:#888;margin-top:2px;">TARGET</div>'
    html += f'</td>'
    html += f'<td style="background:#f8f9fa;padding:10px;border-radius:8px;text-align:center;width:25%;">'
    html += f'<div style="font-size:15px;font-weight:700;color:{stop_color};">${stop:.2f}</div>'
...
def display_flash_cards(matches, potential):
    ...
    if is_breakdown:
        st.markdown('<div class="section-header">📱 Flash Cards - Breakdowns (No BUY Signals)</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="section-header">📱 Flash Cards - Breakouts (No SELL Signals)</div>', unsafe_allow_html=True)
    ...
    for m in matches:
        ...
        # Check if all targets met - skip if so
        ...
        if targets_pending == 0 and len(target_list) > 0:
            continue  # Skip - all targets met
        
        opps.append({
            'symbol': m['symbol'], 'tf': tf, 'type': 'confirmed',
            'return_score': m.get('return_score', 0),
            'max_upside': targets.get('max_upside_pct', 0),
            'max_downside': targets.get('max_downside_pct', 0),
            ...
            'weekly_sell_vol_strong': pattern.get('weekly_sell_vol_strong', False),
            ...
            'breakdowns': len(pattern.get('recent_breakdowns', [])),
            ...
            'has_double_top': pattern.get('has_double_top', False),
            'double_top_confirmed': pattern.get('double_top_confirmed', False),
            'double_top': pattern.get('double_top'),
            'macd_positive': m.get('macd_positive', False),
            'macd_uptick': m.get('macd_uptick', False),
            'macd_negative': m.get('macd_negative', False),
            'macd_downtick': m.get('macd_downtick', False),
    ...
    for p in potential:
        ...
        opps.append({
            ...
            'double_top_confirmed': pattern.get('double_top_confirmed', False),
            'double_top': pattern.get('double_top'),
            'macd_negative': p.get('macd_negative', False),
            'macd_downtick': p.get('macd_downtick', False),
            'tf_data': p.get('tf_data', {}), 'pattern': pattern,
            'is_breakdown': is_breakdown
        })
    
    if not opps:
        mode_text = "breakdown" if is_breakdown else "breakout"
        st.info(f"🔍 Run a scan to see flash cards for top {mode_text} opportunities")
        return
    
    # Sort by return score
    if is_breakdown:
        opps.sort(key=lambda x: (x['return_score'], x.get('max_downside', 0)), reverse=True)
    else:
    ...
    for i, opp in enumerate(opps[:8]):  # Show top 8
        ...
        # Determine status - different for breakdown vs breakout
        if opp['type'] == 'confirmed':
            if is_breakdown:
                # MACD status for breakdown
                if opp.get('macd_negative') and opp.get('macd_downtick'):
                    macd_str = "🔴↓"
                elif opp.get('macd_negative'):
                    macd_str = "🔴"
                elif opp.get('macd_downtick'):
                    macd_str = "📉"
                else:
                    macd_str = ""
                status_text = f"✅ SELL {macd_str}"
                status_color = "red"
            else:
                # MACD status for breakout
        ...
        if is_breakdown:
            yoda_color = "red" if opp['yoda_state'] == 'SELL' else "green"
        else:
        ...
        with st.container():
            ...
            with metrics_col:
                ...
                with perf1:
                    if opp.get('is_breakdown'):
                        st.markdown(f"**:red[-{opp.get('max_downside', 0):.1f}%]**")
                        st.caption("MAX DOWNSIDE")
                    else:
                ...
                if opp.get('is_breakdown'):
                    # Breakdown analysis
                    if opp['type'] == 'confirmed':
                        summary_parts.append(f"**{symbol}** has broken below support with {opp.get('breakdowns', 0)} recent breakdown(s).")
                    else:
                        summary_parts.append(f"**{symbol}** is approaching support, {opp.get('distance', 0):.1f}% away from breakdown.")
                    
                    # Double top pattern
                    if opp.get('has_double_top'):
                        if opp.get('double_top_confirmed'):
                            dt = opp.get('double_top', {})
                            summary_parts.append(f"📊 **Double Top CONFIRMED** with target -{dt.get('target_pct', 0):.1f}%!")
                        else:
                            summary_parts.append("📊 **Double Top forming** - watch for neckline breakdown!")
                    
                    if opp['yoda_state'] == 'SELL':
                        summary_parts.append(f"Yoda shows **SELL** signal (strength: {opp['signal_strength']}).")
                    
                    # Volume analysis for breakdown
                    if opp.get('weekly_sell_vol_strong'):
                        summary_parts.append("📊 **Weekly sell volume strong** - distribution detected!")
                    elif opp.get('volume_confirmed'):
                        summary_parts.append("Volume confirmed on breakdown.")
                else:
                    # Breakout analysis (original)
                    if opp['type'] == 'confirmed':
...
def display_dashboard():
    ...
    if is_breakdown:
        tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["🎯 Creative Dashboard", "📱 Flash Cards", "⚖️ Risk/Reward & Downside", "🏆 Rankings", "🎯 Confirmed Breakdowns", "⏳ Potential Breakdowns", "📊 Full Analysis"])
    else:
        tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["🎯 Creative Dashboard", "📱 Flash Cards", "⚖️ Risk/Reward & Upside", "🏆 Rankings", "🎯 Confirmed Breakouts", "⏳ Potential Breakouts", "📊 Full Analysis"])
...
def display_creative_top(matches, potential, all_results):
    ...
    if is_breakdown:
        st.markdown('<div class="section-header">🎯 Top Opportunities - Breakdowns (No BUY Signals in Daily/Weekly)</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="section-header">🎯 Top Opportunities - Breakouts (No SELL Signals in Daily/Weekly)</div>', unsafe_allow_html=True)
    ...
    for m in matches:
        ...
        # MACD status - different for breakdown
        if is_breakdown:
            macd_negative = m.get('macd_negative', False)
            macd_downtick = m.get('macd_downtick', False)
            if macd_negative and macd_downtick:
                macd_str = "MACD- ↓"
            elif macd_negative:
                macd_str = "MACD-"
            elif macd_downtick:
        ...
        # Target status - different logic for breakdown
        target_list = targets.get('targets', [])
        if is_breakdown:
        ...
        # Skip if all targets met
        if targets_met == len(target_list) and len(target_list) > 0:
            continue
        ...
        for t in target_list:
            if is_breakdown:
                if current_price > t['price']:
                    remaining_pct = ((current_price - t['price']) / current_price) * 100
                    remaining_move = max(remaining_move, remaining_pct)
            else:
                if current_price < t['price']:
        ...
        opps.append({
            ...
            'type': 'confirmed',
            'return_score': m.get('return_score', 0),
            'move_pct': remaining_move if remaining_move > 0 else (targets.get('max_downside_pct', 0) if is_breakdown else targets.get('max_upside_pct', 0)),
            'risk_reward': targets.get('risk_reward', 0),
            'entry': current_price,
            ...
            'volume_strong': pattern.get('weekly_sell_vol_strong', False) if is_breakdown else pattern.get('weekly_buy_vol_strong', False),
            'double_bottom': pattern.get('has_double_bottom', False),
            'double_top': pattern.get('has_double_top', False),
            'tf_data': tf_data,
            'pattern': pattern,
            'is_breakdown': is_breakdown
        })
    ...
    for p in potential:
        ...
        if is_breakdown:
            macd_str = "MACD↓" if p.get('macd_downtick') else ("MACD-" if p.get('macd_negative') else "MACD")
        else:
        ...
        opps.append({
            ...
            'distance': p.get('distance', 0),
            'return_score': pattern.get('return_score', 0),
            'move_pct': targets.get('max_downside_pct', 0) if is_breakdown else targets.get('max_upside_pct', 0),
            'risk_reward': targets.get('risk_reward', 0),
            'entry': targets.get('entry', 0),
    ...
    if not opps:
        mode_text = "breakdown" if is_breakdown else "breakout"
        st.info(f"🔍 Run a scan to discover {mode_text} opportunities")
        return
    
    # Sort by score
    opps.sort(key=lambda x: (x['return_score'], x['move_pct']), reverse=True)
    ...
    if view_mode == "🏆 Top 10":
        ...
        mode_label = "Breakdown" if is_breakdown else "Breakout"
        st.markdown(f"### 🏆 Top 10 {mode_label} Opportunities")
            ...
            with col1:
                st.markdown(
                    opportunity_card_html(
                        ...
                        double_bottom=opp.get('double_bottom', False),
                        is_breakdown=opp.get('is_breakdown', False)
                    ),
                    unsafe_allow_html=True
                )
...
def display_confirmed(matches, all_results=None):
    """Display confirmed breakouts/breakdowns grouped by upside/downside buckets with target status and sector info."""
    ...
    if is_breakdown:
        st.markdown('<div class="section-header">🎯 Confirmed Breakdowns (No BUY in Daily/Weekly)</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="section-header">🎯 Confirmed Breakouts (No SELL in Daily/Weekly)</div>', unsafe_allow_html=True)
    
    if not matches:
        filter_text = "BUY signals" if is_breakdown else "SELL signals"
        mode_text = "breakdowns" if is_breakdown else "breakouts"
        st.info(f"No confirmed {mode_text} found. Stocks with **{filter_text}** in daily or weekly timeframes are excluded.")
        return
    ...
    # View mode selector
    move_label = "📊 Downside Buckets" if is_breakdown else "📊 Upside Buckets"
    view_mode = st.radio("Group By", [move_label, "🏢 Sector"], horizontal=True, key="confirmed_view")
    
    st.markdown("---")
    
    # Process matches to calculate remaining upside/downside and target status
    processed = []
    ...
    for m in matches:
        ...
        # Calculate target status - different logic for breakdown vs breakout
        target_list = targets.get('targets', [])
        targets_met = 0
        targets_pending = 0
        next_target = None
        next_target_pct = 0
        max_remaining_move = 0
        
        for t in target_list:
            if is_breakdown:
                # For breakdown: target met if current price <= target price (price went down)
                if current_price <= t['price']:
                    targets_met += 1
                else:
                    targets_pending += 1
                    # Remaining downside (positive number representing % to fall)
                    remaining_pct = ((current_price - t['price']) / current_price) * 100
                    if next_target is None:
                        next_target = t
                        next_target_pct = remaining_pct
                    max_remaining_move = max(max_remaining_move, remaining_pct)
            else:
                # For breakout: target met if current price >= target price (price went up)
                if current_price >= t['price']:
                    targets_met += 1
                else:
                    targets_pending += 1
        ...
        # Skip stocks that have met ALL targets
        if targets_pending == 0 and targets_met > 0:
            continue  # Filter out - all targets already met
        
        # Use remaining move if targets pending, otherwise use original max
        if is_breakdown:
            move_pct = max_remaining_move if targets_pending > 0 else m.get('max_downside', 0)
        else:
            move_pct = max_remaining_move if targets_pending > 0 else m.get('max_upside', 0)
        ...
        processed.append({
            ...
            'return_score': m.get('return_score', 0),
            'original_move': m.get('max_downside', 0) if is_breakdown else m.get('max_upside', 0),
            'remaining_move': max_remaining_move,
            'move_pct': move_pct,
            'targets_met': targets_met,
            'targets_pending': targets_pending,
            'total_targets': len(target_list),
    ...
    if not processed:
        st.info("All stocks have met their targets - no active opportunities remaining.")
        return
    
    # Define buckets based on mode
    if is_breakdown:
        buckets = [
            {'name': '🚀 30%+ Downside', 'min': 30, 'max': float('inf'), 'items': []},
            {'name': '📉 20-30% Downside', 'min': 20, 'max': 30, 'items': []},
            {'name': '📊 10-20% Downside', 'min': 10, 'max': 20, 'items': []},
            {'name': '📈 5-10% Downside', 'min': 5, 'max': 10, 'items': []},
            {'name': '⚠️ <5% Remaining', 'min': 0, 'max': 5, 'items': []}
        ]
    else:
        buckets = [
    ...
    # Sort into buckets
    for p in processed:
        move = p['remaining_move'] if p['targets_pending'] > 0 else 0
        for bucket in buckets:
            if bucket['min'] <= move < bucket['max']:
    ...
    avg_remaining = np.mean([p['remaining_move'] for p in processed if p['targets_pending'] > 0]) if total_with_targets > 0 else 0
    
    col_summary = st.columns(4)
    col_summary[0].metric("Total Active", len(processed))
    col_summary[1].metric("Targets Pending", total_with_targets, "opportunities")
    
    if is_breakdown:
        col_summary[2].metric("Best Downside", f"-{max((p['remaining_move'] for p in processed), default=0):.1f}%")
        col_summary[3].metric("Avg Remaining", f"-{avg_remaining:.1f}%" if total_with_targets > 0 else "N/A")
    else:
        col_summary[2].metric("Best Upside", f"+{max((p['remaining_move'] for p in processed), default=0):.1f}%")
        col_summary[3].metric("Avg Remaining", f"+{avg_remaining:.1f}%" if total_with_targets > 0 else "N/A")
    
    st.markdown("---")
    
    if move_label in view_mode:
        # Bucket distribution
        cols = st.columns(5)
        ...
        for bucket in buckets:
            ...
            # Sort by remaining move within bucket
            bucket['items'].sort(key=lambda x: (x['targets_pending'] > 0, x['remaining_move']), reverse=True)
        ...
        for i, (sector, sector_items) in enumerate(sorted_sectors[:4]):
            with cols[i]:
                avg_move = np.mean([p['remaining_move'] for p in sector_items if p['targets_pending'] > 0]) if any(p['targets_pending'] > 0 for p in sector_items) else 0
                sector_class = get_sector_class(sector)
                sign = "-" if is_breakdown else "+"
                st.markdown(f"""
                <div style="background: #f8f9fa; padding: 12px; border-radius: 10px; text-align: center;">
                    ...
                    <div style="font-size: 11px; color: #666;">{sign}{avg_move:.1f}% avg</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        for sector, sector_items in sorted_sectors:
            ...
            with st.expander(f"**{sector}** ({len(sector_items)} stocks) — Avg {sign}{avg_move:.1f}% remaining", expanded=len(sector_items) <= 3):
                sector_items.sort(key=lambda x: x['remaining_move'], reverse=True)
...
def _display_confirmed_item(p):
    ...
    is_breakdown = p.get('is_breakdown', False)
    ...
    # MACD status indicator - different for breakdown
    macd_status = ""
    if is_breakdown:
        if p.get('macd_negative') and p.get('macd_downtick'):
            macd_status = "🔴"
        elif p.get('macd_negative'):
            macd_status = "🔴"
        elif p.get('macd_downtick'):
            macd_status = "📉"
    else:
    ...
    if is_breakdown:
        if p.get('weekly_sell_vol_strong'):
            indicators.append("🔥")
        if p.get('double_top_confirmed'):
            indicators.append("M")
    else:
    ...
    # Expander title - use negative sign for breakdown
    ...
    with st.expander(title, expanded=False):
        ...
        with col1:
            ...
            # P&L calculation - different for breakdown (short position)
            if is_breakdown:
                # For short: profit when price goes down
                pnl = ((p['entry_price'] - p['current_price']) / p['entry_price']) * 100 if p['entry_price'] > 0 else 0
            else:
        ...
        with col2:
            ...
            for i, t in enumerate(target_list[:3]):
                if is_breakdown:
                    # For breakdown: target met if current <= target (price went down)
                    if p['current_price'] <= t['price']:
                        st.markdown(f"~~T{t['level']}: ${t['price']:.2f}~~ ✅")
                    else:
                        remaining = ((p['current_price'] - t['price']) / p['current_price']) * 100
                        st.write(f"T{t['level']}: ${t['price']:.2f} (-{remaining:.1f}%)")
                else:
                    # For breakout: target met if current >= target
                    if p['current_price'] >= t['price']:
                        st.markdown(f"~~T{t['level']}: ${t['price']:.2f}~~ ✅")
                    else:
        ...
        with col3:
            ...
            if is_breakdown:
                st.write(f"Remaining: -{p['remaining_move']:.1f}%")
            else:
                st.write(f"Remaining: +{p['remaining_move']:.1f}%")
        ...
        with col4:
            ...
            if is_breakdown:
                if p.get('macd_negative') and p.get('macd_downtick'):
                    st.write("MACD: 🔴 Negative & ↓")
                elif p.get('macd_negative'):
                    st.write("MACD: 🔴 Negative")
                elif p.get('macd_downtick'):
                    st.write("MACD: 📉 Downtick")
                st.write(f"Volume: {'🔥 Strong' if p.get('weekly_sell_vol_strong') else ('✅' if p['volume_confirmed'] else '❌')}")
                if p.get('has_double_top'):
                    st.write(f"Dbl Top: {'✅' if p.get('double_top_confirmed') else '📊'}")
            else:
                if p['macd_positive'] and p['macd_uptick']:
...
def display_potential(potential, all_results=None):
    """Display potential breakouts/breakdowns grouped by move buckets with MACD filter and sector info."""
    ...
    # Dynamic header
    if is_breakdown:
        st.markdown('<div class="section-header">⏳ Potential Breakdowns (No BUY in Daily/Weekly)</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="section-header">⏳ Potential Breakouts (No SELL in Daily/Weekly)</div>', unsafe_allow_html=True)
    
    if not potential:
        filter_text = "BUY signals" if is_breakdown else "SELL signals"
        mode_text = "breakdown setups" if is_breakdown else "breakout setups"
        st.info(f"No potential {mode_text} found. Stocks with **{filter_text}** in daily or weekly timeframes are excluded.")
        return
    ...
    # View mode selector
    move_label = "📊 Downside Buckets" if is_breakdown else "📊 Upside Buckets"
    view_mode = st.radio("Group By", [move_label, "🏢 Sector"], horizontal=True, key="potential_view")
    ...
    for item_id, p in enumerate(potential):
        ...
        p['name'] = info['name']
        p['is_breakdown'] = is_breakdown
        
        # Get move percentage
        if is_breakdown:
            if 'downside' not in p or p.get('downside', 0) == 0:
                pattern = p.get('pattern', {})
                targets = pattern.get('targets', {})
                p['move_pct'] = targets.get('max_downside_pct', 0)
            else:
                p['move_pct'] = p.get('downside', 0)
        else:
            if 'upside' not in p or p.get('upside', 0) == 0:
                pattern = p.get('pattern', {})
                targets = pattern.get('targets', {})
                p['move_pct'] = targets.get('max_upside_pct', 0)
            else:
                p['move_pct'] = p.get('upside', 0)
    ...
    if is_breakdown:
        buckets = [
            ...
            {'name': '📊 10-20% Downside', 'min': 10, 'max': 20, 'items': []},
            {'name': '📈 5-10% Downside', 'min': 5, 'max': 10, 'items': []},
            {'name': '⚠️ <5% Downside', 'min': 0, 'max': 5, 'items': []}
        ]
    else:
        buckets = [
    ...
    if move_label in view_mode:
        ...
        for i, (sector, sector_items) in enumerate(sorted_sectors[:4]):
            with cols[i]:
                avg_move = np.mean([p.get('move_pct', 0) for p in sector_items])
                sector_class = get_sector_class(sector)
                sign = "-" if is_breakdown else "+"
                st.markdown(f"""
                <div style="background: #f8f9fa; padding: 12px; border-radius: 10px; text-align: center;">
        ...
        for sector, sector_items in sorted_sectors:
            avg_move = np.mean([p.get('move_pct', 0) for p in sector_items])
            sign = "-" if is_breakdown else "+"
            move_label = "downside" if is_breakdown else "upside"
            
            with st.expander(f"**{sector}** ({len(sector_items)} stocks) — Avg {sign}{avg_move:.1f}% {move_label}", expanded=len(sector_items) <= 3):
...
def _display_potential_item(p):
    ...
    is_breakdown = p.get('is_breakdown', False)
    move_pct = p.get('move_pct', p.get('downside', p.get('upside', 0)))
    
    # MACD indicators - different for breakdown
    macd_status = ""
    if is_breakdown:
        if p.get('macd_negative') and p.get('macd_downtick'):
            macd_status = "🔴 MACD- & ↓"
    ...
    if is_breakdown:
        type_icon = "📊 DT" if pattern_type == 'double_top' else "📐 TL↓"
    else:
    ...
    # Title with proper sign for breakdown/breakout
    if is_breakdown:
        title = f"{prox} **{p['symbol']}** ({p['timeframe']}) — {dist:.1f}% away | -{move_pct:.1f}% downside | {macd_status}"
    else:
        title = f"{prox} **{p['symbol']}** ({p['timeframe']}) — {dist:.1f}% away | +{move_pct:.1f}% upside | {macd_status}"
    ...
    with st.expander(title):
        ...
        with col1:
            ...
            if is_breakdown:
                st.write(f"Weak Trend: {'✅' if pattern.get('is_unhealthy_trend') else '❌'}")
            else:
        ...
        with col2:
            ...
            if is_breakdown:
                st.write(f"Negative: {'✅' if p.get('macd_negative') else '❌'}")
                st.write(f"Downtick: {'✅' if p.get('macd_downtick') else '❌'}")
                vol_strong = pattern.get('weekly_sell_vol_strong', False)
                st.write(f"Sell Vol: {'🔥' if vol_strong else '📊'}")
            else:
                ...
                st.write(f"Buy Vol: {'🔥' if vol_strong else '📊'}")
    ...
    with st.sidebar:
        ...
        if method == "Manual Entry":
            ...
            if preset_file:
                ...
                possible_paths = [
                    os.path.join(script_dir, "data", preset_file),  # Same dir as script
                    f"/app/data/{preset_file}",  # Docker path
                    f"data/{preset_file}",  # Relative path
                    f"./data/{preset_file}",  # Explicit relative
                    os.path.join(os.getcwd(), "data", preset_file),  # Current working dir
                    preset_file  # Just filename
                ]
                ...
                if not symbols:
                    st.warning(f"⚠️ Could not load preset file '{preset_file}'. Using default watchlist.")
                    st.caption(f"Searched paths: {', '.join(possible_paths[:3])}")
